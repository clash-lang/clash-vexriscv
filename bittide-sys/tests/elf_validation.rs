...
fn example_elf_file_no_segments() {
    ...
    assert!(res.is_err());
    assert_matches!(res.unwrap_err(), ElfValidationError::NoSegmentsFound);
}
...
proptest::proptest! {
    ...
    fn all_elf64_files_invalid(mut info in gen_valid_elf_file()) {
        ...
        assert!(res.is_err());
        assert_matches!(res.unwrap_err(), ElfValidationError::Elf64Found);
    }
}
...
fn all_non_riscv_elfs_invalid(
    ...
    assert!(res.is_err());
    assert_matches!(res.unwrap_err(), ElfValidationError::MachineNotRiscv);
}
...
fn all_non_le_elfs_invalid(#[strategy(gen_valid_elf_file())] mut info: ElfCreateInfo) {
    ...
    let res = validation::validate_elf_file(&bin, &config);
    assert!(res.is_err());
    assert_matches!(res.unwrap_err(), ElfValidationError::UnexpectedEndianess);
}
...
fn all_non_text_entry_invalid(
    ...
    assert_matches!(
        res.unwrap_err(),
        ElfValidationError::EntryPointNotLoaded { entry_point } if entry_point == entry as usize
    );
}
